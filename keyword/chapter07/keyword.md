...
- **낙관적 업데이트(OptimisticUpdate)  🍠**
    ...
    - 어떤 상황에서 **낙관적 업데이트(OptimisticUpdate)가 효율적일까요? 🍠**
        ...
        네트워크 지연은 조금 있지만, 요청이 대부분 성공하고, 실패했을 때 피해가 크지 않은 경우에 낙관적 업데이트가 효율적이라고 생각한다. 
        
        예를 들어 할 일 추가, 좋아요 버튼 클릭, 댓글 작성처럼 사용자의 액션이 자주 발생하고 즉각적인 피드백이 중요한 기능에 적용하면 UX를 크게 개선할 수 있다. 실제로 이런 기능들은 서버에서도 유효성 검사가 단순한 편이라 실패 확률이 낮기 때문에, “어차피 성공할 것이다”라는 가정이 비교적 안전하게 통한다고 느꼈다.
        
        따라서 성공 가능성이 높고, 잠깐 동안 클라이언트↔서버 상태 불일치가 발생해도 큰 문제가 되지 않는 상황에서 낙관적 업데이트가 특히 효과적이라고 생각한다.
        ...
        요청이 실패했을 때의 비용이 크거나, 서버 검증 로직이 복잡해서 성공 여부를 쉽게 가정할 수 없는 경우에는 낙관적 업데이트를 피해야 한다고 생각한다.
        
        예를 들어 결제, 송금, 환불, 재고가 한정된 상품 구매처럼 **금전적 손실**이나 **데이터 정합성 문제**가 치명적인 도메인에서는 UI를 먼저 바꾸는 것이 위험할 수 있다. 또한 권한 변경, 계정 삭제, 중요한 리소스 삭제와 같이 롤백이 까다롭거나 되돌릴 수 없는 작업도 보수적으로 서버 응답을 확인한 뒤에 UI를 업데이트하는 편이 더 안전한 것 같아 보인다. 
        
        그래서 낙관적 업데이트는 “빠른 UX가 가장 중요할 때” 사용하고, “정확성과 신뢰성이 더 중요한 도메인”에서는 의도적으로 피하는 것이 좋다고 생각한다.
    ...
    - **낙관적 업데이트(OptimisticUpdate) 블로그 읽고 개념 정리하기 🍠**
        ...
        - **낙관적 업데이트(OptimisticUpdate)**를 왜 도입해야 하는지, 이 패턴이 해결하려는 문제를 실제 서비스 맥락에서 설명해보세요.
            ...
            낙관적 업데이트를 활용하면 느린 네트워크 환경에서도 어플리케이션의 응답성을 높여, 사용자가 느끼는 느린 속도의 답답함을 감소시켜 줄 수 있기 때문에 도입해야 한다.
            
            특히 빠른 사용자 경험의 제공을 위해 데이터를 서버에 전송한 후 처리되기를 기다리지 않고 UI를 즉시 업데이트시켜 더 빠른 응답성을 제공한다.
            
            이로 인해 사용자는 변경 사항이 즉시 반영된다고 느끼게 되므로 더 자연스러운 어플리케이션 사용이 가능하여, 자연스러운 상호작용이 가능하다. 
            
            그리고 상태가 즉시 반영되므로 비동기 작업 동안 중간 상태를 관리해야하는 부담도 간소화된다.
            
            실제 서비스로 예를 들면, Todo 생성, 좋아요, 댓글작성처럼 사용자가 자주 눌러야 하는 기능을 네트워크가 느릴 때 먼저 ui를 업데이트 해주면 실제 네트워크와 관계없이 매우 앱이 빠르게 동작하는 것처럼 느껴지므로, 사용자 경험이 좋아지는 것에 가장 큰 의의가 있다.
            
        - TanStack Query 기반 구현 흐름을 `onMutate → (mutate) → onError → onSettled` 순서로 기술해주세요..
            1. onMutate: mutate 함수가 호출될 때 가장 먼저 실행. 기존 데이터를 기반으로 UI를 즉시 업데이트하고, 그 이전의 데이터를 저장해둬서 필요시 복원 가능하게 한다.
            2. onError: API 호출이 실패했을 경우 실행. onMutate 단계에서 저장한 이전 데이터를 사용해서 상태를 복원
            3. onSettled: 작업이 성공하거나 실패한 후 항상 호출. 쿼리를 무효화하여 서버 데이터를 최신 상태로 동기화.
        - ToDo “생성” 및 “좋아요 토글”에 **낙관적 업데이트(OptimisticUpdate)**를 적용했을 때의 **실패/충돌 롤백 전략**을 설계해주세요.
            
            ToDo 생성의 경우 UI에서는 새 항목을 먼저 추가하지만, 서버 요청이 실패하면 `previousTodos`로 복원하여 “임시 항목을 제거”하는 형태로 롤백.
            
            좋아요 토글은 양방향 상태 변경이므로 더 면밀한 롤백이 필요하다.
            
             onMutate 단계에서 이전 liked 상태와 likeCount를 정확히 저장해두고, 실패 시 그대로 되돌려야 한다. 또한 좋아요는 연속으로 빠르게 눌릴 수 있어 cancelQueries로 레이스 컨디션을 막는 것이 중요하다. 
            
            ⇒ 생성은 단순 삭제 롤백이면 충분하지만, 좋아요는 상태 충돌 대비가 필수적으로 필요!
...
- [ ]  우측 하단의 **`+`** 버튼 클릭 시 글 작성 모달이 열려야 합니다.
    - [ ]  모달 바깥 영역 클릭 또는 **'X' 버튼** 클릭 시 모달이 닫히도록 사용자 친화적인 UX를 구현해야 합니다.
    ...
    - [ ]  LP **사진을 입력받을 수 있는 `input type="file"`** 형태의 입력 창을 띄워주세요.
...
- [ ]  태그 입력 후  버튼 클릭으로 태그가 추가되어 UI에 표시되어야 합니다.
...
- [ ]  각 태그 옆의 **`x` 버튼**을 누르면 위의 영상 처럼 해당 태그가 즉시 삭제되는 **클라이언트 상태 관리** 로직을 구현해주세요.
- [ ]  **'Add LP'** 버튼 클릭 시, **useMutation**을 이용하여 LP Post API를 호출해야 합니다.
    - [ ]  **POST 요청 성공 시**, 모달이 닫히고 메인 페이지의 LP 목록이 **`onSuccess`** 내부의 `invalidateQueries`를 통해 자동으로 새로고침되어야 합니다.

---

### 댓글 기능

- [ ]  6주차 댓글 레이아웃에 **useMutation**을 연결하여 댓글 작성 API를 호출합니다.
    - [ ]  **댓글 생성 요청 성공 시**, 댓글 목록 **`queryKey`**를 **`invalidateQueries`**하여 작성한 댓글이 목록에 즉시 반영되도록 구현해야 합니다.
...
- [ ]  본인이 작성한 댓글에 한하여 메뉴 버튼 (…버튼)을 누르면 수정 혹은 삭제를 할 수 있는 버튼을 만들고, 수정 기능, 삭제기능도 **useMutation**을 이용하여 구현해주세요!
    ...
    - [ ]  **댓글 수정 요청 성공 시**, 댓글 목록 **`queryKey`**를 **`invalidateQueries`**하여 수정한 댓글 내용이 목록에 즉시 반영되도록 구현해야 합니다.
    - [ ]  **댓글 삭제 요청 성공 시**, 댓글 목록 **`queryKey`**를 **`invalidateQueries`**하여 삭제한 댓글이 목록에 즉시 반영되도록 구현해야 합니다.

---

### 마이 페이지

- [ ]  설정 버튼을 누르면 본인의 이름, bio, 프로필 사진을 수정하는 UI를 띄우고, **useMutation**을 이용하여 수정 기능을 구현해야 합니다.
    ...
    - [ ]  **Bio, 프로필 사진:** 옵션이므로 값이 비어있어도 저장이 가능하도록 처리해야 합니다.

---

### 인증 관련

- [ ]  기존 로그인 로직을 **useMutation**으로 변경해주세요. (회원가입은 다음 주차에서 다룰거라 로그인만 해주시면 됩니다!)
- [ ]  로그인은 서버 상태(인증 토큰)를 **변경**하는 작업이므로 **useMutation**을 사용하며, 성공 시 홈 화면으로 리다이렉션되어야 합니다.
- [ ]  사이드바의 '탈퇴하기' 버튼 클릭 시, **확인 모달**을 띄워 최종 동의를 받아야 합니다.
...
- [ ]  상세 페이지에서 **수정, 삭제** (위에서 구현한 기능 재활용) 및 **좋아요 기능**을 구현해야 합니다. (좋아요는 미션 2로 넘겨서 Optimistic Update를 적용할 예정입니다.)
...
- [ ]  **마이페이지에서 닉네임 변경** 요청 시, 서버의 응답(성공/실패)을 기다리기 전에 `onMutate`를 사용하여 Nav-Bar와 마이페이지의 닉네임을 **즉시 변경**해야 합니다.
...
- [ ]  게시글 상세 조회에서 좋아요를 누를 때 **낙관적 업데이트(OptimisticUpdate)** 를 이용하여 바로 좋아요가 반영되도록 구현해주세요
...
[상태 관리의 이중생활: 클라이언트 vs 서버, 그리고 `useMutation`의 탄생](https://www.notion.so/vs-useMutation-2a6b57f4596b8154aea8c69305fbe2af?pvs=21)
