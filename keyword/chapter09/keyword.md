    ...
    **`Redux Toolkit`**은 처음 접하실 때 다소 복잡하게 느껴질 수 있습니다. 그래서 이번 챕터에서는 여러분의 이해를 돕기 위해 **`Redux Toolkit` 개념 설명을 먼저 제공**해 두었습니다.
    ...
    - **`Redux Toolkit`** 사용법을 공식문서를 보며 직접 정리해보기 🍠
        ...
        - Provider
            ...
            Provider는 리액트 앱 전체에 Redux store를 연결해주는 컴포넌트다.
            이를 통해 모든 하위 컴포넌트에서 useSelector, useDispatch를 사용할 수 있게 된다.
        ...
        - configureStore
            ...
            configureStore는 Redux Toolkit에서 제공하는 스토어 생성 함수다.
            
            여러 slice reducer들을 하나로 합쳐주며, DevTools와 thunk 등 기본 설정이 자동으로 포함된다.
        ...
        - createSlice
            ...
            createSlice는 하나의 기능 단위(state + reducer + action)를 한 번에 묶어서 만드는 방식이다.
            
            Immer가 기본 적용되므로 state를 직접 수정하는 문법처럼 작성해도 불변성이 유지된다.
        ...
        - useSelector
            ...
            useSelector는 store의 값을 조회할 때 사용한다.
            
            컴포넌트는 store를 직접 import하지 않고 항상 useSelector를 통해 값에 접근한다.
        ...
        - useDispatch
            ...
            useDispatch는 액션을 store에 전달할 때 사용한다.
            
            createSlice에서 만든 action creator를 dispatch에 넣어서 사용한다.
        ...
        - 기타 **`Redux Toolkit`** 사용 방법을 상세하게 정리해 보세요
            ...
            비동기 요청을 쉽게 처리하기 위한 도구다.
            
            pending / fulfilled / rejected 상태에 맞는 액션이 자동으로 생성된다.
            
            ### createEntityAdapter
            
            리스트 형태 데이터를 id 기반으로 정리하고 싶을 때 사용한다.
            
            데이터 추가, 삭제, 정렬 등을 효율적으로 관리할 수 있다.
            
            ### RTK Query
            
            데이터 fetching과 캐싱을 자동화하는 도구다.
            
            API와 관련된 로직을 거의 작성하지 않아도 되며, 자동으로 캐싱·리패칭 등이 이루어진다.
...
- **Zustand** 🍠
    ...
    **Zustand** 또한 처음 접하실 때 다소 복잡하게 느껴질 수 있습니다. 하지만 이번 챕터에서는 제가 위에서 **Redux**와 **Redux Toolkit**에 대해 정리해 드린 것 처럼 여러분이 **스스로 탐색하고 정리하는 학습 습관을 기르는 것**을 목표로 하기 때문에, 기본 개념 설명은 따로 제공하지 않았습니다.
    ...
    - **Zustand**란 무엇인가요? 🍠
        ...
        Zustand는 리액트에서 사용할 수 있는 **가벼운 전역 상태 관리 라이브러리**다.
        
        Hook 기반으로 동작하며, 코드 구조가 단순하고 러닝 커브가 낮다는 특징이 있다.
        
        Redux처럼 복잡한 설정이나 Provider가 필요 없으며, 한 파일에서 store를 만들고 바로 사용할 수 있다.
        
        또한 selector 기반 최적화가 기본으로 동작해 불필요한 리렌더링이 잘 발생하지 않는다.
    ...
    - 왜 **Zustand**를 사용할까요? 🍠
        ...
        - 설정이 매우 간단하고 코드가 직관적이다.
        - Provider나 Context 없이 어느 컴포넌트에서도 자유롭게 store를 사용할 수 있다.
        - selector 기반으로 필요한 부분만 구독하므로 리렌더링이 매우 적다.
        - 비동기 로직을 store 내부에 자연스럽게 넣을 수 있다.
        - Persist, Immer 등 여러 미들웨어를 쉽게 추가할 수 있다.
        - 작은 코드 크기와 빠른 성능을 제공한다.
        
        복잡한 구조의 Redux보다 **간단하고 빠른 전역 상태 관리**가 필요할 때 특히 유용하다.
    ...
    - **Zustand**에서 중요한 개념 🍠
        ...
        Zustand store에서 상태를 업데이트할 때 사용하는 함수다.
        set((state) => newState) 형태로 사용하며, immer 없이도 자연스러운 상태 변경 문법을 제공한다.
        
        ### 2) get 함수
        
        현재 store의 상태를 즉시 가져올 때 사용한다.
        
        주로 비동기 로직이나 내부 계산에서 사용된다.
        
        ### 3) 선택적 구독 (selector)
        
        컴포넌트가 store 전체를 구독하지 않고, **필요한 값만** 선택해서 구독하는 방식이다.
    ...
    - **Zustand** 객체 상태 관리 예시 🍠
        ...
        unction Profile() {
          ...
          return (
            <>
              <p>{user.name}</p>
              <button onClick={() => updateName("새 이름")}>변경</button>
            </>
          );
        }
    ...
    - **Zustand** vs Context API 🍠
        ...
        | **구분** | **Zustand** | **Context API** |
        | --- | --- | --- |
        | 목적 | 전역 상태 관리 | 간단한 데이터 전달 |
        | 리렌더링 | 매우 적음 (selector 기반) | Provider 트리 전체가 리렌더링되기 쉬움 |
        | 구조 | store만 만들면 어디서든 사용 가능 | Provider로 감싸야 하며 구조가 깊어질 수 있음 |
        | 비동기 로직 | store 내부에서 자연스럽게 처리 가능 | 별도 로직 필요 |
        | 미들웨어 | Persist, Immer 등 풍부함 | 직접 구현해야 함 |
- **React 전역 상태 관리 완벽 가이드 블로그** 읽고 개념 정리하기 🍠
    ...
    - **`Context API`**의 **`value 전체 구독 메커니즘`**과 **`Zustand`**의 **`selector 기반 구독`**의 성능 차이를 설명해보세요.
        ...
        **Context API**
        ...
        - Context는 `value` 전체를 하나의 덩어리로 취급한다.
        - Provider에서 `value={{ user, theme, ... }}`처럼 객체를 넘기면, 이 객체의 **참조가 바뀌는 순간** `useContext`로 구독하는 모든 컴포넌트가 다시 평가된다.
        - 즉, `user`만 바뀌었든, `theme`만 바뀌었든, **value 객체가 새로 만들어지면** 그 Context를 쓰는 컴포넌트는 전부 다시 렌더링 사이클에 들어간다.
        - 이 때문에 `user만 쓰는 컴포넌트`도 `theme 변경`에 의해 불필요하게 재렌더링될 수 있다. 선택적으로 “user만 구독” 같은 미세 제어가 기본적으로 안 된다.
        
        **Zustand**
        
        - Zustand는 React 바깥에 **module-level store**를 두고, 컴포넌트는 `useStore(selector)` 형태로 **selector를 통해 필요한 조각만 구독**한다.
        ...
        - 내부에서는 `useSyncExternalStore`와 `selector` 결과를 `Object.is`로 비교해, **selector 결과가 진짜로 바뀐 컴포넌트만 재렌더링**한다.
        - 그래서 `user`가 바뀌어도 `theme만 구독하는 컴포넌트`는 그대로고, `theme`가 바뀌어도 `user만 구독하는 컴포넌트`는 영향을 받지 않는다.
        - 결과적으로, Context처럼 “전역적으로 다 같이 흔들리는” 방식이 아니라, **필요한 필드만 세밀하게 구독**하기 때문에 렌더링 비용이 줄어들고 성능이 좋아진다.
        
        요약하면,
        
        - **Context**: `value` 한 번 바뀌면 그 Context를 쓰는 애들이 몰려서 재평가된다.
        - **Zustand**: selector가 뽑아 온 **값이 실제로 바뀐 컴포넌트만** 다시 그린다.
    - **`Jotai`**의 **`atom`** 조합 방식이 파생 상태 관리에서 Zustand 대비 갖는 장점을 의존성 추적 관점에서 설명해보세요.
        
        **Zustand의 파생 상태**
        
        - Zustand에서도 파생 상태를 만들 수 있지만, 보통 **selector 내부에서 계산**하거나, store 밖에서 `useMemo` 등으로 직접 계산한다.
        ...
        - 이 방식은 “지금 이 컴포넌트에서만” 계산하는 느낌이다.
        - 여러 곳에서 같은 파생 로직을 쓰면 **각 컴포넌트마다 계산을 반복**하게 되고, “이 파생 상태가 어떤 기본 상태들에 의존하는지”가 중앙에서 관리되지 않는다.
        - 즉, **의존성 그래프가 명시적으로 관리되는 구조가 아니다**.
        
        **Jotai의 atom 조합 방식**
        
        - Jotai는 상태를 **atom 단위**로 쪼개고, 파생 상태도 또 하나의 atom으로 정의한다.
        ...
        - 여기서 `doubleAgeAtom`은 `get(userAtom)`을 호출하면서, 내부적으로 “나는 `userAtom`에 의존한다”는 정보를 남긴다.
        - Jotai는 이 정보를 기반으로 **의존성 그래프**를 만든다.
            - `userAtom`이 바뀌면 → 이것을 의존하는 `doubleAgeAtom`이 재계산되고 → `doubleAgeAtom`을 구독하는 컴포넌트만 재렌더링된다.
        - 즉, **파생 상태(derived state)를 하나의 1급 상태(atom)로 승격**시키고, 그 사이의 의존 관계를 프레임워크가 자동으로 관리한다.
        
        **장점 정리**
        
        - 파생 상태도 atom이기 때문에,
            - 한 번 정의하면 여러 컴포넌트에서 **공유해서 재사용**할 수 있고
            - 어떤 기본 atom들이 바뀌었을 때 다시 계산해야 하는지 **자동 추적**된다.
        - “어떤 값이 바뀌면 어디까지 다시 계산해야 할까?”를 개발자가 직접 고민할 필요 없이, **의존성 그래프를 통해 최소한만 전파**해 준다.
        - Zustand는 selector 기반이라 “구독 단위” 최적화는 잘 되지만, **파생 상태를 전역적으로 관리하는 데에는 Jotai만큼 구조적이지 않다**는 점에서 차이가 난다.
    ...
    - 서버 상태를 **`useEffect`**로 관리할 때 발생하는 캐싱/중복 요청/불일치 문제를 설명해보세요.
        ...
        - `useEffect`는 보통 컴포넌트 마운트 시마다 API를 호출한다.
        ...
        - 이 컴포넌트가 언마운트되었다가 다시 마운트되면, **항상 새로 요청**을 보낸다.
        - “이미 불러왔던 데이터 재사용(캐싱)”이라는 개념이 기본적으로 없다.
        - 결과적으로 네트워크 낭비 + UX 저하(괜히 다시 로딩 스피너가 뜨는 현상)가 발생한다.
        
        ### 2) 중복 요청 문제
        
        - 같은 데이터를 여러 컴포넌트에서 각각 `useEffect`로 가져오면, **서로 모르게 중복 요청**을 날리게 된다.
        ...
        - 캐시가 없으니, 사용자 정보 한 번 보여주려고도 여러 군데에서 API를 반복 호출하게 된다.
        - 이건 서버 입장에서도 비효율이고, 클라이언트에서도 로딩이 여러 군데서 뜨는 등 지저분한 경험을 만든다.
        
        ### 3) 데이터 불일치(Consistency) 문제
        
        - A 컴포넌트와 B 컴포넌트가 각각 `useEffect`로 서버 데이터를 관리하면,
            - A는 오래된 데이터를 들고 있고
            - B는 최근 데이터를 들고 있는 **불일치 상황**이 쉽게 생긴다.
        - 예를 들어,
            - A에서 `setUser`로 로컬 상태를 업데이트했는데
            - B는 여전히 마운트 시점에 fetch해 둔 예전 데이터를 보여줄 수 있다.
        - “서버에서 갱신된 최신 값”과 “클라이언트가 들고 있는 값”의 동기화를 **모든 컴포넌트마다 직접 관리**해야 해서, 로직이 금방 복잡해진다.
        
        ### 4) 로딩/에러/재시도/무효화까지 직접 관리해야 함
        
        - `useEffect` 방식에서는
            - `isLoading`, `isError`, `errorMessage`를 전부 수동으로 만들고
            - 실패 시 재시도 로직,
            - 특정 시점에 “데이터 만료 → 다시 fetch” 같은 **무효화(invalidation)** 로직까지 직접 구현해야 한다.
        - 서버 상태 전용 라이브러리(TanStack Query, SWR 등)는 이걸
            - 캐싱
            - 중복 요청 방지
            - 백그라운드 refetch
            - 초 단위 stale time 관리
            - 에러/재시도 정책
                ...
                을 **한 번에 해결**해 주지만, `useEffect`만으로 하려면 이 모든 걸 직접 짜야 한다.
    ...
    - **정리**
        - **Context vs Zustand**
            - Zustand는 selector로 **필요한 조각만 구독**해서, 변경된 값과 관련된 컴포넌트만 최소로 다시 그린다.
            - Context는 `value 전체`를 구독해서, 한 필드만 바뀌어도 모든 Consumer가 영향 받는다.
        - **Jotai atom 조합 vs Zustand 파생 상태**
            - Zustand는 파생 상태를 주로 selector 내부에서 “그때그때 계산”하는 느낌이다.
            - Jotai는 파생 상태도 하나의 atom으로 정의하고, `get` 기반으로 **의존성 그래프를 자동으로 관리**해, 어떤 atom이 바뀌면 어떤 파생 atom을 다시 계산할지 체계적으로 추적한다.
        - **서버 상태 + useEffect 문제**
            - 캐싱이 없어서 매번 다시 요청하고,
            - 같은 데이터를 여러 컴포넌트가 중복 요청하고,
            - 각자 따로 상태를 들고 있어 **불일치**가 쉽게 생기며,
            - 로딩/에러/재시도/무효화까지 모두 수동으로 구현해야 한다.
...
- [ ]  `constants` 폴더를 만들고, 그 안에 `cartItems.ts` 파일을 생성해주세요.
- [ ]  주어진 Mock-Data를 `cartItems.ts`에 넣고 이를 활용해주세요.
    - 음반 정보 Mock-Data 파일
...
- [ ]  이 `cartItems`를 Redux 상태의 초기값으로 사용해주세요.

---

### 🎨 2. UI & 스타일링 규칙 (Tailwind CSS)

- [ ]  전체 화면 레이아웃, 카드, 버튼, 텍스트 등을 **Tailwind CSS만 사용해서 구성해주세요.**
- [ ]  음반 리스트, 수량 증가/감소 버튼, 전체 삭제 버튼, 총 수량 및 총 금액 UI를 Tailwind로 만들어주세요.
...
- [ ]  `configureStore`를 사용해서 중앙 저장소(`store`)를 만들어주세요.
- [ ]  `cartSlice`를 `store`의 `reducer`에 추가해주세요.

### 3-2. Provider 연결

- [ ]  앱 전체를 `<Provider>`로 감싸 Redux Store를 연결해주세요.
- [ ]  `<Provider store={store}>` 형태로 적용해주세요.
- [ ]  설정 위치는 보통 `main.tsx` 또는 `index.tsx`에 배치해주세요.
...
3. **아이템 제거 (`removeItem`)**
    - [ ]  특정 음반을 장바구니에서 완전히 제거하는 기능을 만들어주세요.
    - [ ]  **`payload`**로 전달된 `id`를 가진 아이템만 제거해주세요.
4. **전체 삭제 (`clearCart`)**
...
- [ ]  `useSelector`로 Redux의 `cartItems`, `amount`, `total`을 불러와 UI에 표시해주세요.
- [ ]  `useDispatch`로 `increase`, `decrease`, `removeItem`, `clearCart`, `calculateTotals`를 호출해주세요.
- [ ]  수량 버튼 클릭 시 dispatch 해주세요
- [ ]  수량/금액이 변할 때마다 `calculateTotals`가 호출되도록 처리해주세요.

---

### ✅ 6. 최종 동작 점검 체크리스트

- [ ]  장바구니에 Mock 데이터가 정상적으로 보이는지 확인해주세요.
- [ ]  특정 아이템의 **`+`** 버튼 클릭 시:
    - [ ]  해당 음반의 수량이 제대로 증가하는지 확인해주세요.
    - [ ]  전체 수량과 총 금액이 즉시 반영되는지 확인해주세요.
- [ ]  특정 아이템의 **`-`** 버튼 클릭 시:
    - [ ]  해당 음반의 수량이 감소하는지 확인해주세요.
    - [ ]  1 아래로 내려갈 경우 자동 삭제되는지 확인해주세요.
    - [ ]  전체 수량과 금액이 정확히 줄어드는지 확인해주세요.
- [ ]  `전체 삭제` 버튼 클릭 시:
    - [ ]  모든 음반이 리스트에서 제거되는지 확인해주세요.
    - [ ]  전체 수량/금액이 0으로 초기화되는지 확인해주세요.
...
- [ ]  `cartSlice`와 `modalSlice`를 **서로 다른 파일로 분리해서** 만들어주세요.
    
    (예: `features/cart/cartSlice.ts`, `features/modal/modalSlice.ts`)
    
- [ ]  각각의 Slice를 `store`의 reducer에 별도로 등록해주세요.
...
- [ ]  모달을 켜고/끄는 상태 값을 `modalSlice`에서 관리해주세요.
    
    (예: `isOpen: false`)
    
- [ ]  모달을 **열기(open)**, **닫기(close)** 위한 reducer 함수들을 만들어주세요.
    - `openModal()`
    - `closeModal()`
- [ ]  컴포넌트에서는 **useState를 절대 사용하지 않고**,
...
- [ ]  모달이 열렸을 때만 **전체 화면을 덮는 오버레이 레이어의 배경색**을 **어두운 반투명 색상**으로 보일 수 있게 설정해주세요.
- [ ]  모달이 열려야 할 상황에서 `<Modal />` 컴포넌트가 화면에 보여지도록 구현해주세요.
- [ ]  모달에서 **"아니요" 버튼**을 눌렀을 때
...
- [ ]  모달의 모든 열림/닫힘 상태를 **Reducer로만** 제어했는지 확인해주세요.
- [ ]  모달 UI 컴포넌트 내부 및 버튼에서 **useState를 사용하지 않았는지** 확인해주세요.
- [ ]  모달 열림/닫힘에 관련된 로직이 모두 modalSlice에 존재하는지 다시 한번 점검해주세요.

---

### 🧪 5. 최종 동작 테스트

- [ ]  모달 트리거 버튼을 누르면 모달이 정상적으로 등장하는지 확인해주세요.
- [ ]  "아니요" 클릭 → 모달만 닫히는지 확인해주세요.
- [ ]  "네" 클릭 → 모든 장바구니 목록이 삭제되며 모달이 닫히는지 확인해주세요.
...
- [ ]  새로운 Zustand 스토어를 하나 만들어주세요.
    
    (예: `create()`를 사용해서 전역 상태와 액션을 정의해주세요.)
    
- [ ]  기존 Redux `cart` / `playlist` 상태에 담겨 있던 값들을 그대로 Zustand 스토어의 초기값으로 옮겨주세요.
- [ ]  TypeScript를 사용 중이라면, Zustand 상태와 액션에 대한 타입을 직접 정의해 주세요.

---

### 2️⃣ Redux에서 사용하던 액션/리듀서 로직 옮기기

- [ ]  기존 Redux Toolkit에서 사용하던 **증가 / 감소 / 삭제 / 전체 삭제 / 합계 계산** 등의 로직을 Zustand 스토어 안의 액션 함수들로 옮겨주세요.
- [ ]  가능한 한 **동일한 의미의 함수 이름과 매개변수**를 유지해서, 컴포넌트 쪽 수정이 최소화되도록 설계해주세요.
- [ ]  Redux Toolkit에서 `reducer` 안에 있던 “상태 변경 로직”을 이제는 `set((state) => { ... })` 형태로 잘 변환해 주세요.

---

### 3️⃣ 모달(Modal) 관련 상태도 Zustand로 전환하기

- [ ]  기존에 Redux 또는 `useState`로 관리하던 모달의 열림/닫힘 상태를 Zustand 스토어로 옮겨주세요.
- [ ]  “모달 열기 / 닫기”에 해당하는 동작을 별도의 액션 함수로 만들어주세요.
    
    (예: `open`, `close` 같은 이름으로 자유롭게 만들어주세요.)
    
- [ ]  모달 안에서 다른 상태(예: 전체 삭제)와 연결되는 동작이 있다면, Zustand 액션 안에서 서로 잘 연동되도록 구성해 주세요.

---

### 4️⃣ 컴포넌트에서 Redux 흔적 지우고 Zustand로 연결하기

- [ ]  기존에 사용하던 `useSelector`, `useDispatch` 관련 코드를 제거해주세요.
- [ ]  대신, 직접 만든 Zustand 스토어 훅을 호출해서 **상태와 액션을 구조 분해 할당으로 가져와 사용**하도록 바꿔주세요.
    
    (예: `const { items, increase, decrease } = useXXXStore();` 같은 형태로 바꿔주세요.)
    
- [ ]  Redux Provider, Store 설정 등이 더 이상 필요 없으므로, 관련 설정 코드를 깔끔하게 정리해 주세요.

---

### 5️⃣ 동작 동일성 확인하기

- [ ]  Redux Toolkit으로 만들었을 때와 **화면 동작이 완전히 동일한지** 확인해주세요.
- [ ]  개별 아이템의 수량 증가/감소, 삭제, 전체 삭제, 합계 계산 등이 이전과 동일하게 잘 동작하는지 확인해주세요.
- [ ]  모달 열림/닫힘, “네 / 아니요” 시나리오가 Zustand 기반으로도 자연스럽게 동작하는지 확인해주세요.
...
[**React에서 Zustand가 선택받는 이유: 전역 상태 관리의 새로운 시대**](https://www.notion.so/React-Zustand-2b8b57f4596b81cd8749fb97164b0551?pvs=21)
