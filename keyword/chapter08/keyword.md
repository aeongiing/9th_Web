...
- **`Debounce`** 구글링 후 개념 정리 및 코드 작성해보기 🍠
    ...
    - **`Debounce`** 개념 정리 🍠
        ...
        - **마지막 이벤트 이후 일정 시간 동안 아무 이벤트도 없을 때 한 번만 실행**
        - 즉, **사용자가 입력을 "멈추고" 일정 시간이 지나야 실행됨**
        ...
        > 사용자가 계속 요청하면 무시하고,
        > 
        > 사용자가 조용히 멈췄을 때 마지막 요청을 한 번 처리한다.
        > 
        
        ### 👉 예시
        
        - debounce 500ms → 입력이 멈춘 후 0.5초 뒤에 실행
        - 아무리 많이 타이핑해도 끝에 딱 1번 실행됨
        
        ### 👉 언제 사용?
        
        - **끝 상태만 중요하고, 중간 입력은 필요 없는 상황**
        
        예)
        
        - 검색창 자동완성 API 호출
        - 사용자가 타이핑을 멈춘 뒤 필터링 수행
        - 서버로 요청을 배치(batching)해서 보내야 할 때
    - **`Debounce`** 코드 작성 🍠
...
- **`Throttling`** 구글링 후 개념 정리 및 코드 작성해보기 🍠
    ...
    - **`Throttling`** 개념 정리 🍠
        ...
        - **지정된 시간 간격 동안 함수가 최대 한 번만 실행되도록 제한하는 것**
        - 즉, **얼마나 자주 이벤트가 발생하든 일정 주기마다 한 번씩만 실행됨**
        
        ### 👉 비유
        
        > 스로틀은 스프링처럼, 한 번 실행되면 일정 시간 동안 다시 실행될 수 없다.
        > 
        
        ### 👉 예시
        
        - 500ms 스로틀 → 500ms마다 최대 한 번 실행
        - 빠르게 20번 클릭해도 → 함수는 500ms 주기로 1~2번만 실행
        
        ### 👉 언제 사용?
        
        - **지속적으로 발생하는 이벤트를 일정 간격으로 처리해야 할 때**
        - 중간 상태를 계속 처리해도 괜찮은 상황
        
        예)
        
        - 창 크기 변경(resize) 시 UI 업데이트
        - 스크롤하면서 실시간 UI 변경(무한스크롤 position 계산 등)
        - 서버/클라이언트에서 성능이 무거운 작업
    - **`Throttling`** 코드 작성 🍠
...
- [ ]  **`useDebounce`** 커스텀 훅을 직접 만들어보았나요?
    - [ ]  값 지연형 `useDebounce(value, delay)`를 **직접 구현했나요?**
    - [ ]  언마운트/의존성 변경 시 타이머를 **깨끗하게 정리했나요?** (`clearTimeout`)
    - [ ]  지연 시간(`delay`) 변경이 **즉시 반영되도록 처리했나요?**

2) **`useDebounce`** 실제 적용

- [ ]  검색 입력에 **`useDebounce`** 적용
    - [ ]  입력 값을 **`useDebounce`**로 **지연된 값(`debouncedQuery`)으로 만들었나요?**
    - [ ]  빈 문자열/공백만 입력일 때 **요청이 나가지 않도록 막았나요?** (`enabled`)
    - [ ]  지연 시간은 **200–500ms(권장 300ms)** 로 **적절히 설정해 주세요.**
- [ ]  `useInfiniteQuery` 연계
    - [ ]  `queryKey`에 **지연된 값**(`["search", debouncedQuery]`)을 **포함했나요?**
    - [ ]  빈 검색어일 땐 **쿼리가 실행되지 않도록 설정했나요?** (`enabled`)
    - [ ]  `getNextPageParam`으로 **cursor/nextCursor** 기반 페이지네이션을 **구현했나요?**
    - [ ]  `staleTime`/`cacheTime`을 조정해 **불필요한 재요청을 줄여주세요.**

3) **`useDebounce`** 검증

- [ ]  불필요한 서버 요청이 일어나지 않았는지 확인해보셨나요?
    - [ ]  이를 통해 **불필요한 서버 요청을 줄일 수 있었는지** 확인해보세요. (브라우저 Inspect 기능의 네트워크 탭 활용)
...
- [ ]  **`useThrottle`**(value, interval)을 **직접 만들어보셨나요?**
- [ ]  **언마운트/의존성 변경 시 타이머·플래그를 정리**하도록 구현하셨나요? (`clearTimeout` 등)
- [ ]  타입스크립트 사용 시 **제네릭/콜백 파라미터 타입**을 올바르게 지정하셨나요?

2) **`useThrottle`** 실제 적용

- [ ]  **`useThrottle`**을 활용하여 **이벤트가 일정 주기마다 한 번만 실행되도록 제어**하셨나요?
...
- [ ]  브라우저의 **개발자 도구(Inspect → Performance 탭)** 등을 활용해 **`useThrottle` 적용 전후의 성능 차이를 확인**해보셨나요?
- [ ]  **`useThrottle`** 훅이 잘 작동하는 모습을 캡처 영상 또는 GIF로 촬영해 제출해주세요.
...
- [ ]  **Sidebar UI**를 구현하셨나요?
- [ ]  Sidebar가 **자연스럽게 열리고 닫히도록 애니메이션 효과**를 주셨나요?
    - [ ]  CSS Handbook의 Animation 부분을 참고해서 Tailwind CSS의 **Transition 유틸리티 클래스**를 검색하고 적용해 보세요.
...
- [ ]  **Sidebar**의 **열림/닫힘 상태 관리**가 필요하다는 것을 인지하셨나요?
    - [ ]  **Sidebar**를 열 수 있는 **`open()` 함수**를 만드셨나요?
    - [ ]  **Sidebar**를 닫을 수 있는 **`close()` 함수**를 만드셨나요?
    - [ ]  **Sidebar** 상태를 토글할 수 있는 **`toggle()` 함수**를 만드셨나요?
    - [ ]  만든 상태와 함수를 활용해서 햄버거 메뉴 클릭 시 **Sidebar**가 **자연스럽게 열고 닫히는 기능**을 연결하셨나요?
- [ ]  이 상태와 함수들을 **`useSidebar` 커스텀 훅**으로 만들어 재사용성을 확보하셨나요?
    - [ ]  조금 어려우신 분은 Chapter 4 커스텀 훅을 어떻게 만들었었는지 워크북을 통해 확인해보세요.

**3) 접근성 개선: ESC 키로 닫기**

- [ ]  키보드의 **`ESC` 버튼**을 눌러도 Sidebar 컴포넌트가 닫힐 수 있도록 기능을 구현하셨나요?
    - [ ]  **`keydown` EventListener**를 활용해주세요.
    - [ ]  **`useEffect`**의 **클린업 함수**를 통해 EventListener를 등록 해제(취소)하여 메모리 누수를 방지하셨나요?
        - [ ]  이 개념이 헷갈리신다면 **`Chapter 3`** 워크북 **`useEffect`** 부분을 확인해보세요.

**4) 사용성 개선: 배경 스크롤 방지**

- [ ]  사이드바가 열렸을 때 **뒷배경 콘텐츠가 스크롤 되는 현상**이 발생하는지 확인하고 인지하셨나요?
    - [ ]  임의로 긴 텍스트를 넣어 스크롤 현상을 확인해 보셨나요.
- [ ]  이 부분을 어떻게 해결할 수 있을지 한번 직접 AI도 좋고, 검색도 좋고 한번 해결해보세요!
    - [ ]  AI 또는 검색을 통해 이 문제를 **해결**하셨나요?
...
[ **타이핑할 때마다 서버를 공격하지 마세요: Debouncing과 Throttling 실무 이야기**](https://www.notion.so/Debouncing-Throttling-2adb57f4596b816d971bfdea615129fb?pvs=21)
