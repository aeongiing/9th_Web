    ...
    - **`gcTime`**과 **`staleTime`**의 차이점에 대해 정리해보세요! 🍠
        ...
        **위에서 제가 설명해드렸지만 gcTime**과 **staleTime**의 개념을 다시 정리해주시고, 두 값을 어떤 식으로 설정하면 캐싱 전략에 유리한지 설명해주세요!
        ...
        - `gcTime`은 무엇인가요? 🍠
            ...
            gcTime은 가비지 컬렉션 시간으로, 쿼리 데이터가 메모리에서 완전히 제거되기 전까지
            
            유지되는 시간이다.
            즉, 컴포넌트가 언마운트된 뒤에도 gcTime 동안은 캐시가 메모리에 남아 있어서,
            
            동일한 쿼리가 다시 요청되면 네트워크 호출 없이 즉시 재사용할 수 있다.
            
        - `staleTime`은 무엇인가요? 🍠
            
            staleTime은 데이터가 신선하다고 간주되는 시간으로, 이 시간 동안은 리패치가 자동으로 일어나지 않는다.
            예를 들어 staleTime: 5 * 60 * 1000이면, 데이터를 불러온 뒤 5분간은
            
            ‘신선한 데이터’로 인식되어 화면 전환 시에도 재요청이 발생하지 않는다.
            
        - 두 값을 어떤 식으로 설정하여야 `캐싱 전략에 유리`한가요? 🍠
            
            자주 변하지 않는 데이터(예: 사용자 프로필, 설정 정보)는 staleTime을 길게,
            
            gcTime도 충분히 길게 설정하면 불필요한 리패치 없이 빠른 재사용이 가능하다.
            
            반면, 실시간성이 중요한 데이터(예: 주식 시세, 알림 목록)는 staleTime을 짧게,
            
            gcTime은 기본값(5분) 정도로 유지해 최신성을 보장하는 것이 좋다.
            
            요약하자면, staleTime은 데이터의 신선도 기준, gcTime은 메모리 유지 기간으로 구분해 상황에 맞게 조절하는 것이 핵심!
...
- [ ]  기본 레이아웃이 헤더/사이드바/메인이 동시에 노출되도록 구성하셨나요? 레이아웃이 깨지는 뷰포트를 점검하고 CSS를 조정해주세요.
- [ ]  우측 하단 플로팅 버튼(+)이 보이도록 배치하셨나요? 클릭 시 의도한 경로로 라우팅해주세요.
...
- [ ]  비로그인 상태에서 헤더에 “로그인/회원가입” 버튼이 노출되나요? 각각 `/login`, `/signup`으로 라우팅해주세요.
...
- [ ]  로그인 상태에서 “(닉네임)님 반갑습니다.” 문구가 실제 상태 값으로 보이도록 구현하셨나요? 상태값을 연동하고, 필요한 경우 환영 문구 영역을 조건부 렌더링으로 라우팅해주세요.
...
- [ ]  모바일/협소 화면에서 사이드바가 기본 숨김 처리되나요? 버거 버튼 클릭 시 열리도록 구성해주세요.
    - [ ]  아래 버거 아이콘을 활용해주세요.
        - **`SVG`** 버거 코드
...
- [ ]  사이드바 외부 영역을 클릭하면 닫히도록 처리하셨나요?

### 목록 화면(**useQuery** + 정렬)

- [ ]  `/v1/lps` 목록 요청을 `useQuery({ queryKey: ['lps', sort], queryFn })`로 전환하셨나요? 정렬 변경 시 자동 리패치되도록 `sort`를 `queryKey`에 포함해주세요.
- [ ]  정렬 버튼으로 최신순/오래된순이 전환되나요? 토글 로직을 연결하고 결과를 즉시 반영해주세요.
- [ ]  캐시 정책이 의도대로 동작하나요? `staleTime`/`gcTime`을 설정하고 체감을 확인해주세요.
- [ ]  로딩/에러 상태가 사용자에게 명확히 노출되나요? 스켈레톤(또는 스피너)와 재시도 버튼을 제공해주세요.

### 카드 인터랙션/라우팅

- [ ]  카드 Hover 시 확대·오버레이·메타 정보(제목/업로드일/좋아요)가 노출되나요? 스타일을 점검하고 통일해주세요.
...
- [ ]  카드를 클릭하면 해당 `id`로 `/lp/:lpid` 라우팅되나요? 경로 파라미터를 연결해주세요.
...
- [ ]  `/lp/:lpid` 상세를 `useQuery({ queryKey: ['lp', lpid] })`로 패칭하셨나요? 키에 `lpid`를 포함해 주세요.
- [ ]  제목/업로드일/좋아요/썸네일/본문이 구분되어 보기 좋게 렌더되나요? 섹션 레이아웃을 정리해주세요.
- [ ]  수정/삭제/좋아요 버튼이 UI로 배치되어 있나요?
- [ ]  로딩/에러 상태가 목록과 동일 패턴으로 표현되나요? 동일 컴포넌트로 통일해주세요.

### 보호 라우트

- [ ]  비로그인 사용자가 상세 URL 접근 시 경고 모달이 뜨나요? 확인 시 `/login`으로 라우팅해주세요.
...
- [ ]  목록 화면을 **useInfiniteQuery**로 전환하셨나요? `queryKey: ['lps', sort]`, `getNextPageParam`을 구현하고, 트리거에서 `fetchNextPage()`를 호출해주세요.
...
- [ ]  **Skeleton UI**로 로딩을 표현하셨나요? 카드와 동일 크기의 스켈레톤을 렌더하고, `isLoading`/`isFetchingNextPage`에 따라 상단/하단에 각각 표시해주세요. 깜빡이는(쉼머/펄스) 애니메이션을 적용해주세요.
...
- [ ]  `/lp/:LPId` 댓글 목록을 **useInfiniteQuery**로 가져오셨나요? `queryKey: ['lpComments', lpId, order]`로 선언하고, 트리거에서 `fetchNextPage()`로 추가 데이터를 불러와주세요.
- [ ]  오래된순/최신순 정렬이 즉시 반영되나요? `order`를 `queryKey`에 포함하고, 변경 시 첫 페이지부터 다시 로딩하도록 라우팅해주세요.
- [ ]  댓글 작성란은 디자인만 구현하셨나요? 입력 필드·버튼·유효성 안내를 UI로 배치해주세요.
...
- [ ]  댓글 로딩에 **Skeleton UI**를 적용하셨나요? 메인 화면과 동일한 애니메이션을 재사용하고, 초기 로딩은 상단, 추가 로딩은 하단에만 **Skeleton UI**을 표시해주세요.
...
[**TanStack Query vs SWR, 누가 왕좌를 차지할까?**](https://www.notion.so/TanStack-Query-vs-SWR-29bb57f4596b81c19543d6f5b39496cf?pvs=21)
